...
function Cart() {
  ...
  useEffect(() => {
    const addTotal = getCard.reduce((acc,item)=>{
  ...
  const handleIncrement = (id) => {
    ...
    localStorage.setItem("card",JSON.stringify(card1))
    navigate("/cart")
  }


  const handleDecrement = (id) =>  {
    const card2 = getCard.map((item) => {
        if(item.id === id){
            return{
                ...item,quantity:item.quantity - 1
            }
    ...
    localStorage.setItem("card" , JSON.stringify(card2))
    navigate("/cart")
  }

const handleRemove = (id) => {
    const deletecard = getCard.filter((item) => item.id !== id)
    localStorage.setItem("card",JSON.stringify(deletecard))
            ...
            <div className="flex shadow-md my-10">
                <div className="w-3/4 bg-white px-10 py-10">
                    <div className="flex justify-between border-b pb-8">
                        ...
                        <h2 className="font-semibold text-2xl">{getCard.length} items</h2>
                    </div>
                    ...
                    {getCard.map((getCard)=>{
                        ...
                        <div className="flex w-2/5">
                            ...
                            <div className="flex flex-col justify-between ml-4 flex-grow">
                                <span className="font-bold text-sm">{getCard.productname}</span>
                                <span className="text-red-500 text-xs">{getCard.price}</span>
                                <a href="#" onClick={()=>handleRemove(getCard.id)} className="font-semibold hover:text-red-500 text-gray-500 text-xs">Remove</a>
                            </div>
                        </div>
                ...
                <div id="summary" className="w-1/4 px-8 py-10">
                    ...
                    <div className="flex justify-between mt-10 mb-5">
                        <span className="font-semibold text-sm uppercase">Items {getCard.length}</span>
